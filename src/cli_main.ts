...
function shouldAllowUpdateCheck(argv: string[]): boolean {
  const args = argv.slice(2);
  const hasFlag = (flag: string): boolean =>
    args.includes(flag) || args.some((arg) => arg.startsWith(`${flag}=`));

  // Avoid polluting machine-readable output or hooks.
  if (hasFlag('--json') || hasFlag('--quiet') || hasFlag('--hook')) return false;
  if (hasFlag('--help') || hasFlag('-h') || hasFlag('--version') || hasFlag('-v')) return false;
...
cli
  ...
  .option('--json', 'Output as JSON')
  .action(async (options: { days?: number; from?: string; to?: string; json?: boolean }) => {
    try {
      await runReport({
